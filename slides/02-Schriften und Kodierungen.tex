...
\einschub[Tafelgraphik zur Kodierungskette]
\end{frame}
...
\frame{\includegraphics[width=1\textwidth]{unicode}}

\section{Was ist eine Schrift?}
\begin{frame}[fragile]{Was ist eine Schrift?}
• Liste, die einer Zahl eine Glyphe zuordnet
• Eingabe des Codes \macro{0061} muss nicht in einer Glyphe resultieren, die nach einem „a“ aussieht!
...
\• 
\centerline{\includegraphics[width=.3\textwidth]{fontforge.jpeg}}
\einschub[fontforge]
\end{frame}
...
\only<3>{\centerline{\huge\sffamily IV\quad IA}}
\only<4->{\centerline{\rmfamily \huge f\strut i \quad fi}}
\only<5->{Kerning (Unterschneidung) und Ligaturen sind Schrifteigenschaften!}
\only<6->{\\ Im deutschen Satz: max. 3 Buchstaben bilden Ligatur ({\fontspec[Ligatures=Rare]{Arno Pro} ffi, ffl, st})}
\only<7->{\\ Im traditionellen arabischen Satz bis zu 7 Buchstaben!}
\end{frame}
...
\let•\item
\begin{itemize}[<+->]
• Bitmap-Fonts: Kleine Bilder der Buchstaben, Form pixelweise angegeben
\\ ⇒ nicht beliebig vergrößerbar!
• Outline-Schriften:
• Postscript Type1 (pfb/pfm): Beschreibt Buchstabenformen durch Bézierkurven
• TrueType (ttf,ttc): dominierende Schrifttechnologie am Computer (quadratische B-Splines)
• smartfonts: Weiterentwicklungen von TrueType
• OpenType (otf): Weiterentwicklung von Type1 und TrueType mit sehr vielen möglichen Features
...
\begin{frame}{Neo}
\begin{figure}
\only<1>{\includegraphics[width=1\textwidth]{ebene12}
\caption{ Ebenen 1 und 2 der Neo-Tastatur}}
\only<2>{\includegraphics[width=1\textwidth]{ebene3}
\caption{ Ebene 3 der Neo-Tastatur}}
\only<3>{\includegraphics[width=1\textwidth]{ebene4}
\caption{ Ebene 4 der Neo-Tastatur}}
\only<4>{\includegraphics[width=1\textwidth]{ebene5}
\caption{ Ebene 5 der Neo-Tastatur}}
\only<5>{\includegraphics[width=1\textwidth]{ebene6}
\caption{ Ebene 6 der Neo-Tastatur}}
\end{figure}
\uncover<5>{Projektseite: \url{www.neo-layout.org}}
\end{frame}
...
\begin{frame}[fragile]{\XeTeX\, Unicode, OpenType}
...
\begin{frame}[fragile]{\pkg{fontspec}}
